...
export const AllOfficeAmenities: FC<AllOfficeAmenitiesProps> = ({
  accessedAmenities,
}) => {
  const [showLimited, setShowLimited] = useState(true);
  const amenitiesToShow = showLimited
    ? accessedAmenities.slice(0, 4)
    : accessedAmenities;
  return (
      ...
      >
        {amenitiesToShow.map(({ name }) => (
          ...
          >
            ...
            <PrimaryText noWrap fontSize="70%">
              {name}
            </PrimaryText>
          </Grid>
        ))}
