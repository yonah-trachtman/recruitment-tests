...
import { AddEditAssetReq, Status } from '@offisito-shared';
...
export const editAsset = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    if (updatedAsset.assetAmenities) {
      const amens = await amenityModel().find();
      updatedAsset.assetAmenities = updatedAsset.assetAmenities.filter(
        ({ name }) => amens.some((a) => a.name === name),
      );
      await updatedAsset.save();
    }
