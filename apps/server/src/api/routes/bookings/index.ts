...
const trySetCache = (key: TODO, data: TODO) => {
  try {
    cache.set(key, data);
  } catch (e) {
    console.log(e);
  ...
  !userR
    ...
    : await Promise.all(
        bookings.map(async (booking) => {
          ...
          const userX: TODO = await getCachedModelById<User>(
            user(false, false, false) as TODO,
            userR.userType === UserType.host
...
router.get('/', async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    const assetsOfHost = await getCachedAssetsOfHost(String(req.user._id));
...
router.put('/:id', async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    if (booking.guest.toString() !== String(req.user._id))
...
router.put('/read:id', async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    if (!booking) return res.status(404).send('No booking with this id found');
    if (
      !authHost(booking.asset.toString(), String(req.user._id)) &&
      booking.guest.toString() !== String(req.user._id)
    )
      return res.status(401).send('not your booking');
...
router.delete('/decline:id', async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    const booking = await Booking.findById(req.params.id);
    if (!booking) return res.status(404).send('No booking with this id found');
    if (!authHost(booking.asset.toString(), String(req.user._id)))
      return res.status(401).send('not your booking');
